...
method_configs["vanilla-nerfw"] = TrainerConfig(
    ...
    pipeline=VanillaPipelineConfig(
        ...
        model=NeRFWModelConfig(_target=NeRFWModel,
                               ...
                               transient_embed_dim=16,
                               ),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=1e-3, eps=0),
...
method_configs["neuralangelo"] = TrainerConfig(
    ...
    optimizers={
        "fields": {
            ...
            # "scheduler": CosineDecaySchedulerConfig(warm_up_end=5000, learning_rate_alpha=0.05, max_steps=500000),
            "scheduler": MultiStepWarmupSchedulerConfig(warm_up_end=5000, milestones=[300_000, 400_000], gamma=0.1),
        },
        "field_background": {
            "optimizer": AdamWOptimizerConfig(lr=1e-3, eps=1e-15),
...
method_configs["neus-facto-angelo"] = TrainerConfig(
    ...
    steps_per_save=20000,
    steps_per_eval_all_images=1000000,  # set to a very large model so we don't eval with all images
    max_num_iterations=200_001,
    mixed_precision=False,
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            dataparser=SDFStudioDataParserConfig(),
            # load_from_disk=False, cache_compressed_images=True,
...
method_configs["neus-facto-angelo-long"] = TrainerConfig(
    method_name="neus-facto-angelo-long",
    steps_per_eval_image=25000,
    steps_per_eval_batch=25000,
    steps_per_save=20000,
    steps_per_eval_all_images=1000000,  # set to a very large model so we don't eval with all images
    max_num_iterations=1_000_001,
    mixed_precision=False,
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            dataparser=SDFStudioDataParserConfig(),
