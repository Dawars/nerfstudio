...
from nerfstudio.fields.vanilla_nerfw_field import NeRFWField
...
class NeRFWModel(Model):
    ...
    def populate_modules(self):
        ...
        position_encoding = NeRFEncoding(
            in_dim=3, num_frequencies=15, min_freq_exp=0.0, max_freq_exp=8.0, include_input=True
        )
        direction_encoding = NeRFEncoding(
            in_dim=3, num_frequencies=4, min_freq_exp=0.0, max_freq_exp=4.0, include_input=True
        )

        self.field_coarse = NeRFWField(
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.training:
            static_density = field_outputs_fine[FieldHeadNames.DENSITY]
            transient_density = field_outputs_fine[FieldHeadNames.TRANSIENT_DENSITY]
            weights_static = ray_samples_pdf.get_weights(static_density)
            weights_transient = ray_samples_pdf.get_weights(transient_density)
            rgb_static_component = self.renderer_rgb(rgb=field_outputs_fine[FieldHeadNames.RGB], weights=weights_static)
            rgb_transient_component = self.renderer_rgb(
                rgb=field_outputs_fine[FieldHeadNames.TRANSIENT_RGB], weights=weights_transient
            )
            rgb_fine = rgb_static_component + rgb_transient_component
        else:

            weights_static = ray_samples_pdf.get_weights(field_outputs_fine[FieldHeadNames.DENSITY])
            rgb_fine = self.renderer_rgb(rgb=field_outputs_fine[FieldHeadNames.RGB], weights=weights_static)


        accumulation_fine = self.renderer_accumulation(weights_static)
        with torch.no_grad():
            depth_fine = self.renderer_depth(weights_static, ray_samples_pdf)
        ...
        # transients
        if self.training:
            uncertainty = self.renderer_uncertainty(field_outputs_fine[FieldHeadNames.UNCERTAINTY], weights_transient)
            outputs["uncertainty"] = uncertainty + 0.1  # NOTE(ethan): this is the uncertainty min
            outputs["density_transient"] = field_outputs_fine[FieldHeadNames.TRANSIENT_DENSITY]
